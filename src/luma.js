...
export function normalizeLumaUrl(raw) {
  const m = String(raw).trim().match(/^(?:https?:\/\/)?(?:www\.)?(?:lu\.ma|luma\.com)\//i);
  if (!m) return null;
  const slug = String(raw).trim().slice(m[0].length).split(/[/?#\s]/)[0];
...
function ogTags(html) {
  ...
  return { title: pick("og:title"), image: pick("og:image"), url: pick("og:url") };
}
...
export function parseEventPage(html) {
  ...
  const venue =
    (ev?.geo_address_info && ev.geo_address_info.mode !== "obfuscated" && ev.geo_address_info.address)
    || ld?.location?.name
  ...
  return {
    ...
    cover: ev?.cover_url ?? ev?.social_image_url ?? [].concat(ld?.image ?? [])[0] ?? og.image ?? null,
    eventId: typeof ev?.api_id === "string" && ev.api_id.startsWith("evt-") ? ev.api_id : null,
    url: normalizeLumaUrl(ld?.url ?? og.url ?? (ev?.url ? `https://luma.com/${ev.url}` : "")),
  };
}
...
export function formatWhen(startAt, timezone) {
  ...
  if (m[6] && m[6] !== "Z") {
    ...
    try {
      ...
      parts = Object.fromEntries(fmt.formatToParts(new Date(startAt)).map((p) => [p.type, p.value]));
    } catch {
      return null;
    }
  }
...
export function parseCalendarEvents(html) {
  ...
  const looksRight = (xs) => Array.isArray(xs) && xs.some((it) => it?.event?.api_id || it?.event?.url);
  ...
  return items
    .map((it) => {
      ...
      return {
        ...
        slug: u && /^https?:\/\//i.test(u) ? slugOf(u) : slugOf(u ? `https://luma.com/${u}` : null),
